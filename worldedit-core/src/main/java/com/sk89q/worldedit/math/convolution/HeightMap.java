...
public class HeightMap {
    ...
    public int apply(int[] data) throws MaxChangedBlocksException {
        ...
        BlockVector3d minY = region.getMinimumPoint();
        ...
        for (int z = 0; z < height; ++z) {
            for (int x = 0; x < width; ++x) {
                ...
                if (newHeight > curHeight) {
                    ...
                    BlockState existing = session.getBlock(new BlockVector3d(xr, curHeight, zr));

                    // Skip water/lava
                    if (existing.getBlockType() != BlockTypes.WATER && existing.getBlockType() != BlockTypes.LAVA) {
                        session.setBlock(new BlockVector3d(xr, newHeight, zr), existing);
                        ++blocksChanged;

                        // Grow -- start from 1 below top replacing airblocks
                        for (int y = newHeight - 1 - originY; y >= 0; --y) {
                            int copyFrom = (int) (y * scale);
                            session.setBlock(new BlockVector3d(xr, originY + y, zr), session.getBlock(new BlockVector3d(xr, originY + copyFrom, zr)));
                            ++blocksChanged;
                        }
                    }
                    ...
                    session.setBlock(new BlockVector3d(xr, newHeight, zr), session.getBlock(new BlockVector3d(xr, curHeight, zr)));
                    ++blocksChanged;

                    // Fill rest with air
                    for (int y = newHeight + 1; y <= curHeight; ++y) {
                        session.setBlock(new BlockVector3d(xr, y, zr), fillerAir);
                        ++blocksChanged;
                    }
                }
            }
