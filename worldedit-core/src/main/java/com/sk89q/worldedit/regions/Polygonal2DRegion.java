...
public class Polygonal2DRegion extends AbstractRegion implements FlatRegion {
    ...
    private BlockVector2d min;
    private BlockVector2d max;
    private int minY;
    private int maxY;
    ...
    protected void recalculate() {
        if (points.isEmpty()) {
            min = BlockVector2d.ZERO;
            minY = 0;
            max = BlockVector2d.ZERO;
            maxY = 0;
            return;
        }
        ...
        min = new BlockVector2d(minX, minZ);
        max = new BlockVector2d(maxX, maxZ);
    }
    ...
    public void addPoint(Vector2d position) {
        points.add(position.toBlockPoint());
        recalculate();
    }
     ...
     * @param position the position
     */
    public void addPoint(BlockVector3d position) {
    ...
    public BlockVector3d getMinimumPoint() {
        return min.toBlockVector3d(minY);
    }

    @Override
    public BlockVector3d getMaximumPoint() {
        return max.toBlockVector3d(maxY);
    }
     ...
     */
    public static boolean contains(List<BlockVector2d> points, int minY, int maxY, BlockVector3d pt) {
        if (points.size() < 3) {
            return false;
        }
